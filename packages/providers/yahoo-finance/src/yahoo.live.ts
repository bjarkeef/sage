...
import { retryLive as live } from "./live-retry";
 ...
 *
 * Every call goes through `live()`, which retries a failure twice before
 * believing it — see `live-retry.ts` for why, and for the guarantee that a
 * real outage still fails.
 */
describe("Yahoo Finance (live)", () => {
  const provider = new YahooFinanceProvider();

  it("returns a quote", async () => {
    const quote = await live(() => provider.getQuote("AAPL"));
  ...
  it("returns a year of daily bars", async () => {
    ...
    const bars = await live(() => provider.getHistoricalPrices("AAPL", from, to));
  ...
  it("returns dividend history", async () => {
    const dividends = await live(() => provider.getDividendHistory("O"));
  ...
  it("returns an asset profile with a sector", async () => {
    const profile = await live(() => provider.getAssetProfile("O"));
  ...
  it("resolves a non-US symbol, so exchange suffixes still work", async () => {
    ...
    const quote = await live(() => provider.getQuote("VOLV-B.ST"));
